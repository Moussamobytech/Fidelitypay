...
class MerchantPaymentRequest(BaseModel):
    ...
    country: Annotated[str, Field(min_length=2, strict=True, max_length=3)]
    operator: Annotated[str, Field(min_length=0, strict=True, max_length=50)]
    customer: Customer
    return_url: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=1000)]] = Field(default=None, alias="returnUrl")
    cancel_url: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=1000)]] = Field(default=None, alias="cancelUrl")
    __properties: ClassVar[List[str]] = ["amount", "currency", "country", "operator", "customer", "returnUrl", "cancelUrl"]

    @field_validator('currency', mode="before")
    def currency_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if isinstance(value, str) and not re.match(r"XOF", value):
            raise ValueError(r"must validate the regular expression /XOF/")
        return value
